(function (global) {
  ...
  function create(options = {}) {
    ...
    const getReportLocale = options.getReportLocale ?? getReportLanguage;
...
function generateEvidenceAgentMarkdown(data) {
  const markdown = reportUi.generateFullRepairMarkdown(data, getReportLanguage());
  return getReportLocale() === 'zh-Hant' ? global.GeoScoreI18n?.toTraditionalMarkdown?.(markdown) ?? markdown : markdown;
}
...
` : '';
  ...
  const markdown = `# SEO & GEO repair checklist — ${domain}
