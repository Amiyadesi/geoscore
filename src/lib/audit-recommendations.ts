...
function recommendationCopy(checkItem: NormalizedCheck, zh: boolean) {
  ...
  const copies: Record<string, { title: string; why: string; fix: string; verify: string }> = {
    ...
    'geo.author_signal': zh
      ? { title: '明确内容责任归属', why: '适用的文章页没有可核验的作者、责任发布方，也没有可信的个人站点实体承担内容责任。', fix: '按真实发布方式标明作者或责任发布方。个人博客可以维护可信的站点级 Person 实体，不要为了通过检查在非文章页硬加作者。', verify: '检查文章最终 HTML 中的可见署名、author、publisher 或可信站点 Person 是否与页面事实一致，然后重新审计该文章。' }
      : { title: 'Clarify content responsibility', why: 'An applicable article has no verifiable author, responsible publisher, or trusted personal-site entity that can take responsibility for the content.', fix: 'Identify a truthful author or responsible publisher for the article. A personal blog may maintain a trusted site-level Person instead of adding an artificial byline to non-article pages.', verify: 'Inspect the final HTML for a visible byline, author, publisher, or trusted site Person that matches the published facts, then re-audit the article.' },
